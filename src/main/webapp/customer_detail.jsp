...
<body class="hold-transition sidebar-mini">
	<div class="wrapper"><jsp:include page="navbar.jsp" /><jsp:include page="sidebar.jsp" /><div class="content-wrapper">
			<div class="content-header">
				<div class="container-fluid d-flex justify-content-between">
					...
					<div>
						<a href="${pageContext.request.contextPath}/customer/edit?id=${customer.id}" class="btn btn-outline-primary">
							<i class="fas fa-edit"></i>
							编辑
						</a>
						<a href="${pageContext.request.contextPath}/follow/add?customerId=${customer.id}" class="btn btn-primary">
									...
									<div class="row">
										<div class="col-md-3">
											...
											<div class="detail-value">
												<span class="badge badge-info">${customer.levelName}</span>
											</div>
										</div>
										<div class="col-md-3">
											<div class="detail-label">行业 / 规模</div>
											<div class="detail-value">${customer.industry}/ ${customer.scale}</div>
										</div>
										<div class="col-md-3">
										...
										<div class="col-md-6">
											...
											<div class="detail-value">${customer.province}${customer.city}${customer.address}</div>
										</div>
										<div class="col-md-3">
										...
										<div class="col-md-3">
											...
											<div class="detail-value">${empty customer.lastFollowTime?'从未跟进':customer.lastFollowTime}</div>
										</div>
										<div class="col-12">
						...
						<div class="col-lg-4">
							<div class="card">
								...
								<div class="card-body">
									...
									<button class="btn btn-outline-primary btn-block" data-toggle="modal" data-target="#transferModal">
						...
						<div class="col-lg-6">
							<div class="card">
								<div class="card-header">
									...
									<a href="${pageContext.request.contextPath}/contact/add?customerId=${customer.id}" class="float-right">
										...
										<tbody>
											<c:forEach items="${contactList}" var="x">
												<tr>
													...
													<td>${x.phone}</td>
													<td>
														<c:if test="${x.isPrimary==1}">
															<span class="badge badge-info">主联系人</span>
														</c:if>
														<c:if test="${x.isDecisionMaker==1}">
															<span class="badge badge-warning">决策人</span>
														</c:if>
													</td>
												</tr>
											</c:forEach>
											<c:if test="${empty contactList}">
												<tr>
													<td colspan="4" class="text-center text-muted py-4">暂无联系人</td>
												</tr>
											</c:if>
										</tbody>
									</table>
						...
						<div class="col-lg-6">
							<div class="card">
								<div class="card-header">
									...
									<a href="${pageContext.request.contextPath}/opportunity/add?customerId=${customer.id}" class="float-right">
										<i class="fas fa-plus"></i>
										新增
									</a>
								</div>
									...
									<table class="table mb-0">
										...
										<tbody>
											<c:forEach items="${opportunityList}" var="x">
												<tr>
													<td>
														<a href="${pageContext.request.contextPath}/opportunity/detail?id=${x.id}">${x.title}</a>
													</td>
													<td>
														<span class="badge badge-info">${x.stageName}</span>
													</td>
													<td>
														¥
											...
											<c:if test="${empty opportunityList}">
												<tr>
													<td colspan="3" class="text-center text-muted py-4">暂无商机</td>
												</tr>
											</c:if>
										</tbody>
									</table>
									...
									<div class="timeline-simple">
										<c:forEach items="${followList}" var="x">
											<div class="timeline-item-simple">
												<div class="timeline-time">${x.followTime}· ${x.followUserName} · ${x.followType}</div>
												<strong>
							...
							<div class="card">
								...
								<div class="card-body">
									<c:forEach items="${transferList}" var="x">
										<div class="mb-3">
											<small class="text-muted">${x.transferTime}</small>
											<div>${empty x.fromUserName?'未分配':x.fromUserName}
									...
									<c:if test="${empty transferList}">
										<span class="text-muted">暂无转移记录</span>
									</c:if>
								</div>
							</div>
						</div>
	...
	<div class="modal fade" id="transferModal">
		<div class="modal-dialog">
			<form class="modal-content" action="${pageContext.request.contextPath}/customer/transfer" method="post">
				...
				<div class="modal-body">
					<div class="form-group">
						<label>新负责人 *</label>
						<select name="toUserId" class="form-control" required>
	...
	</div><jsp:include page="scripts.jsp" /></body>
</html>
