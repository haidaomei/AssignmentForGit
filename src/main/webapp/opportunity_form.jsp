            ...
            <div class="wrapper">
                ...
                <div class="content-wrapper">
                    ...
                    <section class="content">
                        <div class="container-fluid">
                            <form method="post" action="${pageContext.request.contextPath}/opportunity/save"
                                id="oppForm"><input type="hidden" name="id" value="${opportunity.id}">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="form-group col-md-6"><label>商机标题 *</label><input name="title"
                                                    value="${opportunity.title}" class="form-control" required></div>
                                            <div class="form-group col-md-3"><label>客户 *</label><select
                                                    name="customerId" id="customerId" class="form-control" required>
                                                    <option value="">请选择</option>
                                                    <c:forEach items="${customerList}" var="c">
                                                        <option value="${c.id}" ${opportunity.customerId==c.id||(empty
                                                            opportunity&&selectedCustomerId==c.id)?'selected':''}>
                                                            ${c.customerName}</option>
                                                    </c:forEach>
                                                </select></div>
                                            <div class="form-group col-md-3"><label>主要联系人</label><select
                                                    name="contactId" id="contactId" class="form-control"
                                                    data-selected="${opportunity.contactId}">
                                                    <option value="">请先选择客户</option>
                                                </select></div>
                                            <div class="form-group col-md-3"><label>当前阶段 *</label><select name="stageId"
                                                    class="form-control" required>
                                                    <c:forEach items="${stageList}" var="s">
                                                        <option value="${s.id}" ${opportunity.stageId==s.id||(empty
                                                            opportunity&&s.sortOrder==1)?'selected':''}>
                                                            ${s.name}（${s.probability}%）</option>
                                                    </c:forEach>
                                                </select></div>
                                            <div class="form-group col-md-3"><label>预计成交日期</label><input type="date"
                                                    name="estimatedCloseDate" value="${opportunity.estimatedCloseDate}"
                                                    class="form-control"></div>
                                            <div class="form-group col-md-3"><label>负责人</label><select
                                                    name="ownerUserId" class="form-control"
                                                    ${sessionScope.user.sales?'disabled':''}>
                                                    <c:forEach items="${userList}" var="u">
                                                        <option value="${u.id}" ${(empty
                                                            opportunity&&u.id==sessionScope.user.id)||opportunity.ownerUserId==u.id?'selected':''}>
                                                            ${u.realName}</option>
                                                    </c:forEach>
                                                </select>
                                            ...
                                            <div class="form-group col-12"><label>商机描述</label><textarea
                                                    name="description" class="form-control"
                                                    rows="3"><c:out value="${opportunity.description}"/></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="card">
                                    <div class="card-header">
                                        <h3 class="card-title">产品明细</h3><button type="button"
                                            class="btn btn-sm btn-outline-primary float-right" id="addRow"><i
                                                    ...
                                                    <c:forEach items="${opportunity.items}" var="item">
                                                        <tr>
                                                            ...
                                                            <td><input name="items[][quantity]" class="form-control qty"
                                                                    ...
                                                                    required></td>
                                                            <td><input name="items[][unitPrice]"
                                                                    class="form-control price" type="number" min="0"
                                                                    step="0.01" value="${item.unitPrice}" required></td>
                                    ...
                                    <div class="card-footer text-right"><a
                                            href="${pageContext.request.contextPath}/opportunity/list"
                                            class="btn btn-light">取消</a> <button class="btn btn-primary"><i
                                                class="fas fa-save"></i> 保存商机</button></div>
                                </div>
                            </form>
                        </div>
                    </section>
