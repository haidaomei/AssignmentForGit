										...
										<select name="customerId" id="customerId" class="form-control" required>
											...
											<c:forEach items="${customerList}" var="c">
												<option value="${c.id}" ${opportunity.customerId==c.id||(empty
                                                            opportunity&&selectedCustomerId==c.id)?'selected':''}>${c.customerName}</option>
											</c:forEach>
										</select>
									</div>
									<div class="form-group col-md-3">
										...
										<select name="stageId" class="form-control" required>
											<c:forEach items="${stageList}" var="s">
												<option value="${s.id}" ${opportunity.stageId==s.id||(empty
                                                            opportunity&&s.sortOrder==1)?'selected':''}>${s.name}（${s.probability}%）</option>
											</c:forEach>
										</select>
									</div>
									<div class="form-group col-md-3">
										...
										<select name="ownerUserId" class="form-control" ${sessionScope.user.sales?'disabled':''}>
											<c:forEach items="${userList}" var="u">
												<option value="${u.id}" ${(empty
                                                            opportunity&&u.id==sessionScope.user.id)||opportunity.ownerUserId==u.id?'selected':''}>${u.realName}</option>
											</c:forEach>
										</select>
